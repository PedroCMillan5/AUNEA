...
function addRisk(){
  ...
  openModal('Añadir riesgo',`<div class="step-groups"><details class="step-group" open><summary>Riesgo</summary><div class="form-grid"><div class="field"><label>Categoría</label><select id="riskCat"><option value="">Selecciona…</option>${cats.map(o=>`<option value="${attr(o.value)}">${esc(o.label)}</option>`).join('')}</select></div><div class="field"><label>Descripción concreta</label><input id="riskDesc" placeholder="Evento/consecuencia material"></div><div class="field"><label>Probabilidad 1–5</label><select id="riskLike">${[1,2,3,4,5].map(x=>`<option value="${x}">${x}</option>`).join('')}</select></div><div class="field"><label>Impacto 1–5</label><select id="riskImpact">${[1,2,3,4,5].map(x=>`<option value="${x}">${x}</option>`).join('')}</select></div></div></details><details class="step-group"><summary>Controles y resto</summary><div class="form-grid"><div class="field"><label>Reversibilidad</label><select id="riskRev">${rev.map(o=>`<option value="${attr(o.value)}">${esc(o.label)}</option>`).join('')}</select></div><div class="field"><label>Controles actuales</label><select id="riskControls"><option value="1">Presentes</option><option value="0">Ausentes / insuficientes</option></select></div><div class="field"><label>Datos sensibles / alto impacto</label><select id="riskSensitive"><option value="${sensitive.length?'1':'0'}">${sensitive.length?'Sí — reutilizado de DF073':'No indicado en DF073'}</option><option value="1">Sí</option><option value="0">No</option></select></div><div class="field"><label>Financiero/compliance material</label><select id="riskMat"><option value="0">No</option><option value="1">Sí</option></select></div><div class="field"><label>Trigger crítico</label><select id="riskCritical"><option value="0">No</option><option value="1">Sí</option></select></div></div></details></div>`,()=>{
