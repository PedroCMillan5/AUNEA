...
function bindCrm(){
  ...
  document.querySelectorAll('[data-contact-select-option]').forEach(el=>el.onclick=e=>{e.preventDefault();e.stopPropagation();const id=el.dataset.contactSelectOption,target=document.getElementById(id);if(!target)return;target.value=el.dataset.value||'';const box=el.closest('details.aunea-select'),label=box?.querySelector(`[data-contact-select-label="${id}"]`);if(label)label.textContent=el.textContent.trim();box?.querySelectorAll('[data-contact-select-option]').forEach(btn=>btn.classList.toggle('selected',btn===el));if(box)box.open=false});
