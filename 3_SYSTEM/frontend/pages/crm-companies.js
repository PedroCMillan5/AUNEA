...
function companyInspector(co){
  ...
  if(active==='Contactos'){
    ...
    const list=(state.opportunities||[]).filter(o=>o.companyId===co.id);body=list.length?`<div class="result-list">${list.map(o=>{const linkedStudies=state.engagements.filter(e=>e.opportunityId===o.id),hasStudy=linkedStudies.length>0,hasProject=linkedStudies.some(e=>state.projects.some(p=>p.engagementId===e.id));return `<div class="result-item"><div class="result-item-head"><div><b>${esc(o.title||'Oportunidad')}</b><p>${esc(o.stage||'—')}${hasProject?' · Proyecto creado':hasStudy?' · Estudio creado':''} · ${esc(o.source||'—')}</p></div><div class="result-actions">${inspectorAction('Editar',`data-company-edit-opportunity="${attr(o.id)}"`)}${hasStudy?'':inspectorAction('Crear estudio',`data-company-study-opportunity="${attr(o.id)}"`,true)}</div></div></div>`}).join('')}</div>`:'<div class="empty"><p>Sin oportunidades registradas.</p></div>';
...
function companiesPage(){
  ...
  return `<span class="companies-screen-marker" hidden></span>`+pageTop('Empresas','Gestiona las empresas con las que trabaja AUNEA.',`<button class="btn btn-primary" id="addCompanyBtn">+ Nueva empresa</button>`,'I90-00-01')+workspace(main,companyInspector(co),{wide:true})+actionBar(left,right);
}
