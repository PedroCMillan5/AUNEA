...
function uatPage(){
  ...
  return pageTop('UAT / QA','Pruebas aisladas: no escriben empresas, contactos, estudios, proyectos ni outputs en la base operativa.',`<button class="btn" id="exportBackup">Exportar backup</button><button class="btn" id="importBackup">Restaurar backup</button><button class="btn btn-primary" id="runVisibleUAT">Ejecutar UAT</button>`)+summary+
...
async function runVisibleUAT(){
  ...
  }catch(err){toast('Error ejecutando UAT: '+err.message);if(btn){btn.disabled=false;btn.textContent='Ejecutar UAT'}}
}
