...
function diagnosticFieldMeta(f){return `<span class="canonical-id">${f.Field_ID}</span>${f.Requiredness==='REQUIRED_90M'?requiredMark():''}`}
function habitualVolumeBlock(f21,f22,e){
  ...
  return `<div class="question-card volume-pair" data-uat="UAT-VIS-023"><div class="question-head"><div><div class="question-title">Volumen habitual</div><div class="question-purpose">${esc(f21.Objetivo_concreto||'')} ${esc(f22.Objetivo_concreto||'')}</div></div><div class="question-meta">${diagnosticFieldMeta(f21)} ${diagnosticFieldMeta(f22)}</div></div><div class="question-body"><div class="compound-control volume-sentence"><input type="number" min="0" step="any" data-number-value="${f21.Field_ID}" value="${attr(value.value??'')}" placeholder="120"><input type="hidden" data-number-unit="${f21.Field_ID}" value="case"><span class="unit-label">casos por</span><select data-answer="${f22.Field_ID}"><option value="">periodo…</option>${periods.map(o=>`<option value="${attr(o.value)}" ${String(period)===String(o.value)?'selected':''}>${esc(String(o.label).toLowerCase())}</option>`).join('')}</select></div></div><div class="field-help"><b>Ejemplo:</b> ${esc(f21.Ejemplo_ES||'120 casos')} por ${esc(f22.Ejemplo_ES||'mes')} · <b>Validación:</b> ${esc(f21.Validation||'')} ${esc(f22.Validation||'')}</div></div>`;
}
function peakVolumeBlock(f,e){
  const value=numberParts(effectiveValue(f,e)),periods=fieldOptions('OS_PERIOD');
  return `<div class="question-card volume-pair" data-uat="UAT-VIS-024"><div class="question-head"><div><div class="question-title">${esc(f.Pregunta_o_etiqueta_ES)}</div><div class="question-purpose">${esc(f.Objetivo_concreto||'')}</div></div><div class="question-meta">${diagnosticFieldMeta(f)}</div></div><div class="question-body"><div class="compound-control volume-sentence"><input type="number" min="0" step="any" data-number-value="${f.Field_ID}" value="${attr(value.value??'')}" placeholder="200"><input type="hidden" data-number-unit="${f.Field_ID}" value="case"><span class="unit-label">casos por</span><select data-number-period="${f.Field_ID}"><option value="">periodo…</option>${periods.map(o=>`<option value="${attr(o.value)}" ${String(value.period)===String(o.value)?'selected':''}>${esc(String(o.label).toLowerCase())}</option>`).join('')}</select></div></div><div class="field-help"><b>Ejemplo:</b> ${esc(f.Ejemplo_ES||'220 casos/mes')} · <b>Validación:</b> ${esc(f.Validation||'')}</div></div>`;
}
...
function stagePage(){
  ...
  // Distinct id from validationSummary's own #runDiag CTA (rendered below, inside this same page, on the
  // last stage): two elements sharing one id would leave getElementById('runDiag') bound to only the
  // first in document order (this header button), silently disabling the click on the big CALCULAR CTA.
  return pageTop('Diagnóstico guiado','Cuestionario canónico en español, generado desde Diagnostic Master v1. Una información se captura una vez y se reutiliza después.',`<button class="btn" data-goto-process="1">Ver mapa AS-IS</button><button class="btn btn-primary" id="runDiagHeader">${e.diagnosticOutput?'Recalcular':'Calcular diagnóstico y recomendación'}</button>`) +
  `<div class="stage-layout"><aside class="card stage-nav">${summary}${schema.flow.map((s,i)=>`<button class="stage-btn ${s.Stage_ID===stage.Stage_ID?'active':''}" data-stage="${s.Stage_ID}"><span class="stage-num">${i+1}</span><span><b>${esc(s.Stage_ES)}</b><small>${esc(s.Objetivo)}</small></span><span class="stage-time">${s.Minutos_objetivo}m</span></button>`).join('')}</aside><div><div class="card stage-card"><div class="section-title"><div><h2>${stageIndex+1}. ${esc(stage.Stage_ES)}</h2><p>${esc(stage.Objetivo)}</p></div><div class="chip gold">${stage.Minutos_objetivo} min objetivo</div></div><div class="stage-meta"><span class="chip">Salida: ${esc(stage.Salida)}</span><span class="chip">Criterio: ${esc(stage.Criterio_de_salida)}</span></div>${fields.some(f=>f.Requiredness==='REQUIRED_90M')?REQUIRED_LEGEND_HTML:''}<div class="notice"><strong>Hilo conductor:</strong> ${esc(stage.Interacción_principal)}. ${esc(stage.Regla_de_tiempo)}</div><div style="margin-top:12px">${renderStageFields(fields,e)}</div>${stage.Stage_ID==='S04'?processPrompt(e):''}${stage.Stage_ID==='S05'?frictionPrompt(e):''}${stage.Stage_ID==='S06'?riskBuilder(e):''}${stage.Stage_ID==='S07'?economicBuilder(e):''}${isLastStage?validationSummary(e,completion):''}<div class="stage-footer"><button class="btn" id="prevStage" ${stageIndex===0?'disabled':''}>← Anterior</button><div class="stage-progress-secondary field-help">${stageStat.applicable} campo(s) aplicables en esta etapa · ${stageStat.answered} con dato</div>${isLastStage?'':'<button class="btn btn-primary" id="nextStage">Siguiente →</button>'}</div></div></div></div>`
}
