...
AUNEA Internal V1 está en `REVIEW` con runtime Master v1.1, renderer, No-Reask, AS-IS/fricciones, adapter oficial a motores, modos Sesión/Interno, persistencia recuperable y UAT visible implementados. Pain → Economics → Risk → Recommendation → Pricing → Scenario permanece server-owned. El acceptance gate automatizado pasa; la promoción a `main` queda condicionada al UAT visual/nativo final.
...
| AUNEA-UAT-GATE-010 | .github/workflows/aunea-v1-gate.yml | Gate conjunto backend/frontend sobre el mismo commit. | PROJECT_RULES / acceptance criteria | repo head | CI status | GitHub Actions | CRITICAL | 22 backend + 23 frontend | REVIEW |
